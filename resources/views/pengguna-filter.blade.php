        ...
        <div class="intro-y box mt-5">
            ...
            <form action="{{ route('pengguna.filter') }}" method="post">
                ...
                <div
                    ...
                    <a href="{{ route('cetak.cetak_label', $data_ruangan->id) }}" target="_blank"
                        class="btn btn-warning w-40 ms-2">Cetak Label</a>
                    <a href="{{ route('cetak.cetak_dokumen', $data_ruangan->id) }}" target="_blank"
                        class="btn btn-danger w-52 ms-2">Cetak Dokumen</a>
                </div>
            </form>
                            ...
                            <tbody>
                                @forelse ($details as $index => $detail)
                                    <tr>
                                        ...
                                        <td>{{ $detail->status }}</td>
                                        <td>
                                            <a href="{{ route('cetak.cetak_satuan', $detail->id) }}" target="_blank"
                                                class="btn btn-success me-1 mb-2"> <i data-feather="maximize"
                                                    class="w-5 h-5"></i>
                                            </a>
                                            @if ($detail->status == 'Perbaiki')
                                                <a href="{{ route('pengguna.perbaiki', $detail->id) }}" class="btn btn-warning me-1 mb-2"
                                                    disabled> <i data-feather="tool"
                                                        class="w-5 h-5"></i>
                                                </a>
                                            @else
                                                <a href="{{ route('pengguna.perbaiki', $detail->id) }}"
                                                    class="btn btn-warning me-1 mb-2"> <i data-feather="tool"
                                                        class="w-5 h-5"></i>
                                                </a>
                                            @endif
                                        </td>
